...
import com.head.picker.adapter.FolderAdapter;
import com.head.picker.adapter.ImageAdapter;
import com.head.picker.entry.Folder;
import com.head.picker.entry.Image;
import com.head.picker.entry.RequestConfig;
import com.head.picker.model.ImageModel;
import com.head.picker.utils.DateUtils;
import com.head.picker.utils.ImageSelector;
import com.head.picker.utils.ImageUtil;
import com.head.picker.utils.UriUtils;
import com.head.picker.utils.VersionUtils;
...
public class ImageSelectorActivity extends AppCompatActivity
{
    ...
    private static final int PERMISSION_WRITE_EXTERNAL_REQUEST_CODE =
        0x00000011;
    ...
    {
        Intent intent =
            new Intent(fragment.getActivity(), ImageSelectorActivity.class);
        intent.putExtra(ImageSelector.KEY_CONFIG, config);
     ...
     * @param config
     */
    public static void openActivity(android.app.Fragment fragment,
        int requestCode, RequestConfig config)
    {
        Intent intent =
            new Intent(fragment.getActivity(), ImageSelectorActivity.class);
        intent.putExtra(ImageSelector.KEY_CONFIG, config);
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState)
    {
        ...
        RequestConfig config =
            intent.getParcelableExtra(ImageSelector.KEY_CONFIG);
    ...
    private void initImageList()
    {
        ...
        ((SimpleItemAnimator)rvImage.getItemAnimator())
        ...
        mAdapter
            .setOnImageSelectListener(new ImageAdapter.OnImageSelectListener()
            {
                @Override
                public void OnImageSelect(Image image, boolean isSelect,
                    int selectCount)
                {
    ...
    private void initFolderList()
    {
        ...
        {
            isInitFolder = true;
            rvFolder.setLayoutManager(
                new LinearLayoutManager(ImageSelectorActivity.this));
            FolderAdapter adapter =
                new FolderAdapter(ImageSelectorActivity.this, mFolders);
            adapter.setOnFolderSelectListener(
                new FolderAdapter.OnFolderSelectListener()
                {
                    @Override
    ...
    private void setSelectImageCount(int count)
    {
        ...
        else
        {
            ...
            tvPreview.setText(
                getString(R.string.selector_preview) + "(" + count + ")");
            if (isSingle)
            {
                tvConfirm.setText(R.string.selector_send);
            }
            ...
            {
                tvConfirm.setText(getString(R.string.selector_send) + "("
                    + count + "/" + mMaxCount + ")");
            }
            else
            {
                tvConfirm.setText(
                    getString(R.string.selector_send) + "(" + count + ")");
            }
        }
    }
    ...
    private void openFolder()
    {
        if (!isOpenFolder)
        {
            ...
            ObjectAnimator animator = ObjectAnimator
                .ofFloat(rvFolder, "translationY", rvFolder.getHeight(), 0)
                .setDuration(300);
    ...
    private void closeFolder()
    {
        if (isOpenFolder)
        {
            ...
            ObjectAnimator animator = ObjectAnimator
                .ofFloat(rvFolder, "translationY", 0, rvFolder.getHeight())
                .setDuration(300);
            animator.addListener(new AnimatorListenerAdapter()
            {
                @Override
    ...
    private void hideTime()
    {
        if (isShowTime)
        {
            ObjectAnimator.ofFloat(tvTime, "alpha", 1, 0)
                .setDuration(300)
                .start();
            isShowTime = false;
        }
    ...
    private void showTime()
    {
        if (!isShowTime)
        {
            ObjectAnimator.ofFloat(tvTime, "alpha", 0, 1)
                .setDuration(300)
                .start();
            isShowTime = true;
        }
    ...
    private void saveImageAndFinish(final ArrayList<String> images,
    ...
    private void toPreviewActivity(ArrayList<Image> images, int position)
    {
        ...
        {
            PreviewActivity.openActivity(this,
                images,
        ...
        Intent data)
    {
        ...
        {
            if (data != null
                && data.getBooleanExtra(ImageSelector.IS_CONFIRM, false))
            {
                ...
                if (photoFile != null)
                {
                    ...
                    if (VersionUtils.isAndroidN())
                    {
                        ...
                        photoUri = FileProvider.getUriForFile(this,
                            getPackageName() + ".imageSelectorProvider",
                            photoFile);
                    }
                    else
                    {
        ...
        throws IOException
    {
        ...
        if (!Environment.MEDIA_MOUNTED
            .equals(EnvironmentCompat.getStorageState(tempFile)))
        {
            return null;
        }
        return tempFile;
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event)
    {
        if (keyCode == KeyEvent.KEYCODE_BACK
            && event.getAction() == KeyEvent.ACTION_DOWN && isOpenFolder)
        {
            closeFolder();
            return true;
        }
