...
public class ImageModel
{
    ...
    public static void preloadAndRegisterContentObserver(final Context context)
    {
        ...
        {
            observer =
                new PhotoContentObserver(context.getApplicationContext());
     ...
     * @param callback
     */
    public static void loadImageForSDCard(final Context context,
     ...
     * @param callback
     */
    private static void loadImageForSDCard(final Context context,
        final boolean isPreload, final DataCallback callback)
    {
            ...
            public void run()
            {
                synchronized (ImageModel.class)
                {
                    ...
                    if (cacheImageList == null || isPreload)
                    {
                        ...
                        {
                            // 过滤不存在或未下载完成的图片
                            boolean exists = !"downloading"
                                .equals(getExtensionName(image.getPath()))
                                && checkImgExists(image.getPath());
                            // 过滤剪切保存的图片；
                            boolean isCutImage = ImageUtil
                                .isCutImage(imageCacheDir, image.getPath());
    ...
    {
        return UriUtils.getPathForUri(context,
            MediaStore.Images.Media.EXTERNAL_CONTENT_URI.buildUpon()
                .appendPath(String.valueOf(id))
                .build());
    }
     ...
     * @return
     */
    private static ArrayList<Folder> splitFolder(Context context,
                                                 ArrayList<Image> images)
    {
        ArrayList<Folder> folders = new ArrayList<>();
        folders.add(
            new Folder(context.getString(R.string.selector_all_image), images));
