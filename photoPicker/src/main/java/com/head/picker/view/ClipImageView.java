...
public class ClipImageView extends AppCompatImageView
{
    ...
    public Bitmap clipImage()
    {
        ...
        Bitmap targetBitmap = Bitmap
            .createBitmap(mTargetWidth, mTargetHeight, Bitmap.Config.ARGB_8888);
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                else
                {
                    ...
                    if (distance > 10f)
                    {
                        ...
                        float temp_right = (temp_left
                            + mBitmapWidth * temp_values[Matrix.MSCALE_X]);
                        float temp_bottom = (temp_top
                            + mBitmapHeight * temp_values[Matrix.MSCALE_Y]);
                        // 靠边预判断
                        if (temp_left > mCircleX
                            || temp_right < mCircleX + mTargetWidth
                            || temp_top > mCircleY
                            || temp_bottom < mCircleY + mTargetHeight)
                        {
                            return true;
                        }
                        mMatrix.postScale(scale,
                            scale,
                            mMiddlePoint.x,
                            mMiddlePoint.y);
                        mLastDistance = getDistance(event);
